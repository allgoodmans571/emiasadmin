...
export default function Stages(props) {
    ...
    async function handleActStages() {
        ...
        let data = {actual_stage: Number(document.querySelector('#actStage').value)}
        try {
    ...
    async function handleTotalStages() {
        ...
        let data = {total_stage: Number(document.querySelector('#totalStages').value)}
        try {
    ...
    return(
        ...
        <div className="input-group mb-3" style={{marginBottom: 0, width: '20%'}}>
            ...
            <div className="input-group-prepend">
            <span className="input-group-text" id="inputGroup-sizing-default" >Актуальная стадия</span>
            </div>
        ...
        <button className='btn-success'onClick={handleTotalStages}> Отправить Всего стадий </button> <br />
        <button className='btn-success'onClick={() => handleActStages()}> Отправить Актальная стадия </button>
        </div>
